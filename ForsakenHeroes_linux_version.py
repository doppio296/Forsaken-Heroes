...
# Used to resize the screenshot for Boss Key
...
class StarPlatinum(Hero):
    ...
    def firstEffect(self):
        subprocess.Popen(["aplay timeStop.wav"], shell=True)

    def secondEffect(self):
        subprocess.Popen(["aplay timeStart.wav"], shell=True)
...
class KingCrimson(Hero):
    ...
    def firstEffect(self):
        subprocess.Popen(["aplay timeErase.wav"], shell=True)

    def secondEffect(self):
        subprocess.Popen(["aplay timeLeap.wav"], shell=True)
...
class Mandom(Hero):
    ...
    def firstEffect(self):
        subprocess.Popen(["aplay timeWillRevert.wav"], shell=True)

    def secondEffect(self):
        subprocess.Popen(["aplay timeWasReverted.wav"], shell=True)
...
class CrazyDiamond(Hero):
    ...
    def effect(self):
        subprocess.Popen(["aplay healing.wav"], shell=True)
...
def wallMightExplode():
    ...
    if (exploding == 0 and len(ToBeDetonated) > 0):
        ...
        if (event[0] != realTime):
            ...
            subprocess.Popen(["aplay explosionBefore.wav"], shell=True)
    ...
    if (exploding == 1 or (0 < explodingTime < 18)):
        ...
        if (explodingTime == 2):
            ...
            detonate(X, Y)
            subprocess.Popen(["aplay explosionAfter.wav"], shell=True)
                ...
                for k in range(4, 8):
                    ...
                    if (wall[X][Y] > 0 and grid[X][Y] ==
                        ...
                        if (random.randint(1, 100) <
                            ...
                            subprocess.Popen(
                                ["aplay explosionBefore.wav"], shell=True)
...
# Used to resize the screenshot for Boss Key
fitted = Image.open("hide.png")
fitted = fitted.resize((width, height), Image.ANTIALIAS)
hidePNG = ImageTk.PhotoImage(fitted)
...
nanePNG = PhotoImage(file="nane.png")
